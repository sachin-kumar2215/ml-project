...
class CNNSentimentClassifier(nn.Module):
    ...
    def __init__(self, vocab_size: int, embed_dim: int, output_dim: int,
                 filter_sizes: List[int] = [3, 4, 5], num_filters: int = 100, dropout: float = 0.3):
        super(CNNSentimentClassifier, self).__init__()

        # Embedding layer: Converts word indices to dense vectors
        self.embedding = nn.Embedding(vocab_size, embed_dim, padding_idx=0) # padding_idx=0 for <PAD> token

        # Convolutional layers with different filter sizes
        # nn.ModuleList is used to hold multiple nn.Module objects
        self.convs = nn.ModuleList([
            nn.Conv1d(in_channels=embed_dim, out_channels=num_filters, kernel_size=size)
