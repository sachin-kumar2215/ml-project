import pandas as pd # <--- THIS LINE IS CRUCIAL
import torch
...
if __name__ == "__main__":
    ...
    # The 'original_comment' column is not needed for the predict_sentiment function itself,
    # as it now correctly handles the 'combined_comments' input.
    # If you want to preserve the original comments for display in the main block, you can keep it.
    # For the function's input, 'combined_comments' is sufficient.

    df_with_sentiment = predict_sentiment(dummy_df_inference, artifacts)

    print("\nInference Results:")
    print(df_with_sentiment[['combined_comments', 'Overall_Comment_Sentiment', 'Sentiment_Prob_positive', 'Sentiment_Prob_neutral', 'Sentiment_Prob_negative']].head())