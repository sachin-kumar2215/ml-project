...
def load_sentiment_artifacts(config: InferenceConfig) -> Dict[str, Any]:
    ...
    if not os.path.exists(config.vocab_path):
        raise FileNotFoundError(
            f"Vocabulary file not found at {config.vocab_path}. Please train the model first."
        )
    ...
    if not os.path.exists(config.label_map_path):
        raise FileNotFoundError(
            f"Label map file not found at {config.label_map_path}. Please train the model first."
        )
...
def predict_sentiment(df: pd.DataFrame, artifacts: Dict[str, Any]) -> pd.DataFrame:
    ...
    prob_df = pd.DataFrame(
        all_probabilities,
        columns=[
            f"Sentiment_Prob_{reverse_label_map[i]}"
            for i in range(sentiment_cfg.num_classes)
        ],
    )
    ...
    df_output["Overall_Comment_Sentiment"].fillna(
        "neutral", inplace=True
    )  # Default for unpredicted
    for col in [
        "Sentiment_Prob_negative",
        "Sentiment_Prob_neutral",
        "Sentiment_Prob_positive",
...
if __name__ == "__main__":
    ...
    if (
        not os.path.exists("models/best_cnn_model.pth")
        or not os.path.exists("models/vocabulary.pkl")
        or not os.path.exists("models/label_map.json")
    ):
        print(
            "Error: Model artifacts not found. Please run trainer.py or main.py in 'train' mode first."
        )
        exit()
    ...
    dummy_df_inference["combined_comments"] = (
        dummy_df_inference[["Comment 1", "Comment 2"]].fillna("").agg(" ".join, axis=1)
    )
    dummy_df_inference["original_comment"] = dummy_df_inference[
        "combined_comments"
    ]  # For merging back
