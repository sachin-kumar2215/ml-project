...
class DataPreprocessor:
    ...
    def clean_text(self, text: str) -> str:
        ...
        tokens = [self.lemmatizer.lemmatize(token) for token in tokens
                 if token not in self.stop_words and len(token) > 2] # Filter out very short words
...
def download_and_load_data(gcs_url: str, local_path: str = 'data/raw/social_media_data.csv') -> pd.DataFrame:
    """
...
def extract_and_process_comments(df: pd.DataFrame, preprocessor: DataPreprocessor) -> pd.DataFrame:
    """
    ...
    for idx, row in df.iterrows():
        for col in existing_comment_cols:
            ...
            if pd.notna(comment_text) and str(comment_text).strip() != "":
                ...
                if cleaned_comment and len(cleaned_comment.split()) >= 2: # Ensure cleaned comment is meaningful
                    ...
                    comment_data.append({
                        'post_id': row['Post_ID'], # Assuming 'Post_ID' exists in your raw data
...
def save_processed_data(df: pd.DataFrame, path: str = 'data/processed/processed_social_media_data.csv'):
    """
...
if __name__ == "__main__":
    ...
    print(english_comments[['original_comment', 'cleaned_comment', 'final_sentiment']].head())